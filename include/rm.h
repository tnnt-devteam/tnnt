...
#define ACCESSIBLE(typ) ((typ) >= DOOR && (typ) != NKI) /* good position */
...
/* TNNT - "graffitied" is applied to random graffiti squares (which are always
 * of typ ROOM) upon generation, then zeroed when you step on them, for purposes
 * of counting the number of graffiti you've seen but not letting you get credit
 * for the same graffiti twice. */
#define graffitied flags
