...
enum digcheck_result {
    ...
    /* TNNT: addition for non-kitten item */
    DIGCHECK_FAIL_NKI
};
...
struct multishot {
    ...
    short o;
    boolean s;
    struct obj *obj; /* TNNT - for avoiding crash in deathmatch when opponent is
                      * killed via a multishot, transient object that there are
                      * still more of in hero's inventory */
};
