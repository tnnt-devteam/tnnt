...
out_container(struct obj *obj)
{
    ...
    if (gc.current_container->otyp == SWAP_CHEST
        && gc.current_container->swapitems < SWAP_ITEMS_MIN) {
        /* should not get to here if we haven't contributed to the chest */
        You_feel(gc.current_container->swapitems == SWAP_CHEST_USED_UP
    ...
    boolean more_containers) /* True iff #loot multiple and this isn't last */
{
    ...
    if (gc.current_container->otyp == SWAP_CHEST) {
        refresh_swap_chest_contents(gc.current_container);
    }
    ...
    for (;;) { /* repeats iff '?' or ':' gets chosen */
        ...
        if (c == '?') {
            ...
            /* TNNT Swap chest --> */
            if (gc.current_container->otyp == SWAP_CHEST) {
                if (gc.current_container->swapitems < SWAP_ITEMS_MIN) {
 ...
 containerdone:
    if (used) {
        ...
        if (gc.current_container && gc.current_container->otyp != SWAP_CHEST)
            gc.current_container->cknown = 1;
        update_inventory();
    }
