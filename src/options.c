...
staticfn int handler_realtime(void);
staticfn int handler_realtime_format(void);
    ...
    char *opts, char *op)
{
    ...
    if (req == do_set) {
        ...
        if (op != empty_optstr && !negated) {
            ...
            for (i = 0; i < SIZE(realtime_type_strings); ++i) {
                /* match against first character only */
                if (op[0] == realtime_type_strings[i][0]) {
                    iflags.show_realtime = op[0];
                    return optn_ok;
                }
            }
            return optn_err; /* failed to match */
        }
        else if (negated) {
            bad_negation(allopt[optidx].name, TRUE);
            return optn_err;
    ...
    if (req == do_handler) {
        return handler_realtime();
    }
    return optn_ok;
}

staticfn int
optfn_realtime_format(
    int optidx, int req, boolean negated,
    char *opts, char *op)
{
    int i;
    ...
    if (req == do_set) {
        op = string_for_opt(opts, negated);
        if (op != empty_optstr && !negated) {
            iflags.realtime_format = 'f';
            for (i = 0; i < SIZE(realtime_format_strings); ++i) {
                /* match against first character only */
                if (op[0] == realtime_format_strings[i][0]) {
                    iflags.realtime_format = op[0];
                    return optn_ok;
                }
            }
            return optn_err; /* failed to match */
    ...
    if (req == do_handler) {
        return handler_realtime_format();
    }
    return optn_ok;
}
