    ...
    unsigned int trflags)
{
    if (mtmp == &gy.youmonst) {
        ...
        if (wearing_iron_shoes(mtmp)) {
            ...
            int changed = newcham(mtmp, (struct permonst *) 0, NC_SHOW_MSG);
            if (in_sight)
                seetrap(trap);
            if (changed && mtmp->mtame)
...
blow_up_landmine(struct trap *trap)
{
    ...
    /* TNNT - blowing up a mine in Ludios will necessarily prevent them from
     * achieving "disarm all mines in Ludios", so track that here. */
    if (Is_knox(&u.uz))
    ...
    while (dist-- > 0 && !used_up) {
        ...
        if (dist > 0 && isok(x + dx, y + dy)) {
            ...
            if (typ == IRONBARS) {
                ...
                /* object stops here but with no message */
                x2 = x, y2 = y;
            }
        }
    } /* while dist > 0 */
