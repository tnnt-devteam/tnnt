...
restgamestate(NHFILE *nhfp)
{
    ...
    Sfi_tnnt_globals_t(nhfp, &tnnt_globals, "gamestate-tnnt_globals");
    /* TNNT - restore dynamically allocated things (based on globals) */
    if (tnnt_globals.num_planes_pets > 0) {
        ...
        for (i = 0; i < tnnt_globals.num_planes_pets; ++i) {
            Sfo_unsigned(nhfp, &tnnt_globals.planes_pet_m_ids[i],
                         "gamestate-planes_pet_m_id");
        }
    }
    else {
