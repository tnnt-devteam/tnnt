    ...
    boolean portal)    /* True if arriving via magic portal */
{
    ...
    if (!In_hell(&u.uz0) && Inhell) {
        ...
        tnnt_achieve(A_ENTERED_GEHENNOM);
    }
    ...
    if (In_endgame(&u.uz)) {
        ...
        if (new && Is_minetown(&u.uz)) {
             ...
             * Watch) then the achievement for clearing out all the watchmen
             * can't be obtained this game, which is unfortunate but better than
             * awarding it for doing nothing beyond randomly getting a minetown
             * with no watch.
