...
revive(struct obj *corpse, boolean by_hero)
{
    ...
    mtmp->mrevived = REVIVED;
    if (by_hero)
        mtmp->mrevived |= REVIVED_BY_HERO;
           ...
           int bounceback)
{
    if (!*ddx || !*ddy || (bounceback > 0 && !rn2(bounceback))) {
        ...
        *ddy = -(*ddy);
         ...
         * achievement won't be granted after this occurs (of course, if it got
         * to 3 bounces prior to a wild bounce, or somehow has enough range to
         * get 3 more bounces after a wild bounce, it will still count) */
        tnnt_globals.total_bounces = 0;
    } else {
        uchar rmn;
        int bounce = 0;
        coordxy lsy = sy - *ddy;
    ...
    boolean forcemiss)
{
    ...
    tnnt_globals.total_bounces = 0; /* TNNT: for achievement */
...
break_statue(struct obj *obj)
{
    ...
    while ((item = obj->cobj) != 0) {
        if (item->oclass == SPBOOK_CLASS && !has_omonst(obj))
