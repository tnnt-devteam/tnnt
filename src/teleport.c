        ...
        if (!tnnt_globals.deathmatch_completed) {
            ...
            for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
                if (is_deathmatch_opponent(mtmp)) {
                    /* this duplicates livelog_mon_nam because that macro
                     * definition is local to mon.c and this is the only place
                     * we need it outside that file */
                    mname = x_monnam(mtmp, ARTICLE_THE, (char *) 0,
