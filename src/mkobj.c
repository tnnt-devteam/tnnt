...
mksobj_init(struct obj **obj, boolean artif)
{
    ...
    case FOOD_CLASS:
        ...
        case CANDY_BAR:
            ...
            /* TNNT: track candy that generated in Sokoban;
             * note it doesn't conflict with candy wrapper since soko_candy
             * aliases usecount */
            if (In_sokoban(&u.uz) && gi.in_mklev)
                otmp->soko_candy = 1;
            break;
        default:
            break;
    ...
    if (obj1 && obj2) {
        ...
        if (otmp1 && otmp2 && otmp1 != otmp2) {
            ...
            if (o1wt <= 300 && o2wt <= 300 && otmp1->owt > 300) {
                /* 301 is the weight threshold in xname_flags() to be considered
