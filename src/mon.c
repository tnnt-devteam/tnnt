...
staticfn void tnnt_arena_victory(struct monst *);
...
movemon_singlemon(struct monst *mtmp)
{
     ...
     * Note the placement of this block: after DEADMONSTER and mon_offmap so a
     * recently killed or removed demon lord will not work for the achievement,
     * but before the movement check because it is not required that all of them
     * actually get a turn.
     * Checking it here in movemon() precludes awarding the achievement when the
     * last one of them appears (if that happens on a fast hero action and the
     * hero has more actions to do something before the demon lords do), but is
     * a more performant way to do it than adding a separate loop over fmon
     * somewhere else. */
    if (!mtmp->mpeaceful
...
{
    gs.somebody_can_move = FALSE;
    /* TNNT - reset this for Boss Rush achievement */
    tnnt_globals.dlords_on_level = 0;
...
logdeadmon(struct monst *mtmp, int mndx)
{
    ...
    /* TNNT-specific achievement/livelog cases start here */
    else if (mtmp->data->msound == MS_NEMESIS
               && !tnnt_globals.nemesis_got_a_move) {
    ...
    /* TNNT code for achievements that trigger when a monster dies (NOT when the
...
/* TNNT: this is also used for devteam members responding to an attack on
 * another devteam member */
staticfn void
qst_guardians_respond(void)
{
    ...
    int got_mad = 0;
    boolean dt_level = Is_devteam(&u.uz);

    if (dt_level)
        q_guardian = &mons[PM_DEVTEAM_MEMBER];
...
setmangry(struct monst *mtmp, boolean via_attack)
{
    ...
    if (mtmp->data == &mons[quest_info(MS_LEADER)]
        || mtmp->data == &mons[PM_DEVTEAM_MEMBER])
        qst_guardians_respond(); /* TNNT: also handles devteam */
