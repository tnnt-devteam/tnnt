...
savebones(int how, time_t when, struct obj *corpse)
{
 ...
 make_bones:
    ...
    /* TNNT: include special level variant number in bones, so that the check
     * where Orcish Town is ineligible for the clear-watchmen-from-Minetown
     * achievement will work correctly when a player gets a bones Orcish Town.
     */
