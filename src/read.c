...
doread(void)
{
    ...
    if (!seffects(scroll)) {
        ...
        scroll->in_use = FALSE;
        if (otyp != SCR_BLANK_PAPER && otyp != SCR_MISSING_CODE)
            useup(scroll);
    }
    return ECMD_TIME;
}
...
/* TNNT - effects (cosmetic only) of reading scroll of missing code */
staticfn void
seffect_missing_code(struct obj **sobjp UNUSED)
{
    if (objects[SCR_MISSING_CODE].oc_name_known) {
