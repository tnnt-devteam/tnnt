    ...
    struct obj *obj)    /* obj is not NULL */
{
    ...
    /* TNNT:
     * test that the monster is not in the first 2 spaces of the flight path
     * (which would be a direct strike); in this function, we already know we
     * are not hitting in melee (despite the confusing and unintuitive function
     * names) because !hmd->thrown && is_missile (true for boomerangs) goes to
     * hmon_hitmon_weapon_ranged  */
    ...
    if (obj->oartifact) {
        ...
        if (((weap->spfx & SPFX_DMONS)
                && (mon->data == &mons[(int) weap->mtype]))
            || ((weap->spfx & SPFX_DCLAS)
                && (weap->mtype == (unsigned long) mon->data->mlet))
            || ((weap->spfx & SPFX_DFLAG1)
                && ((mon->data->mflags1 & weap->mtype) != 0L))
            || ((weap->spfx & SPFX_DFLAG2)
                && ((mon->data->mflags2 & weap->mtype) != 0L))) {
            hmd->artibane = TRUE;
        }
    }
    if (obj->oartifact
    ...
    int dieroll)
{
    ...
    if (hmd.dmg >= mon->mhpmax && hmd.dmg >= TNNT_OHKO_DMG)
    ...
    int specialdmg) /* blessed and/or silver bonus against various things */
{
    ...
    mdef->mhp -= mhm.damage;
    if (mhm.damage >= mdef->mhpmax && mhm.damage >= TNNT_OHKO_DMG)
        tnnt_achieve(A_ONE_HIT_KO);
    if (DEADMONSTER(mdef)) {
