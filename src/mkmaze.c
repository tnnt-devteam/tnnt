...
staticfn void tnnt_setup_rfk_level(void);
...
makemaz(const char *s)
{
    ...
    if (*s) {
        if (sp && sp->rndlevs) {
            ...
            Snprintf(protofile, sizeof protofile, "%s-%d", s, level_variant);
            /* TNNT: for letting us distinguish orctown from other minetowns */
